    ...
    async def listBirthdays(self, ctx):
        ...
        for i in data:
            if system.hid == i[0]:
                ...
                if len(birthdays) > 0:
                    birthdays.sort(key = lambda d: (parser.isoparse(d.birthday).month, parser.isoparse(d.birthday).day, (d.display_name if d.name_privacy == "private" else d.name)))
                    for i in range(1,13):
                        ...
                        if len(new_birthdays) > 0:
                            ...
                            for j in range(1,32):
                                new_new_birthdays = [b for b in new_birthdays if parser.isoparse(b.birthday).day == j]
